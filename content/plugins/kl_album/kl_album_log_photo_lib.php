...
function addPhoto(url,width,height,alt)
{
	if (parent.KE.g['content'].wyswygMode == false){
		alert('请先切换到所见所得模式');
	}else if (url != "") {
		parent.KE.insertHtml('content','<a target=\"_blank\" href=\"'+url+'\"><img src=\"'+url+'\" width=\"'+width+'\" height=\"'+height+'\" title=\"点击查看原图\" alt=\"'+alt+'\" border=\"0\"></a>');
	}
}
function addPhotoNew(url,width,height,alt)
{
	if (parent.editorMap['content'].designMode === false){
		alert('请先切换到所见所得模式');
	}else if (url != "") {
		parent.editorMap['content'].insertHtml('<a target=\"_blank\" href=\"'+url+'\"><img src=\"'+url+'\" width=\"'+width+'\" height=\"'+height+'\" title=\"点击查看原图\" alt=\"'+alt+'\" border=\"0\"></a>');
	}
}
</script>
<body>
...
if(!$album)
{
	foreach ($log_photo_info as $value)
	{
		...
		if(isset($value['head']))
		{
			...
			{
				$log_photo_cover = substr($log_photo_row[2], strpos($log_photo_row[2], 'content/uploadpic/'), strlen($log_photo_row[2])-strpos($log_photo_row[2], 'content/plugins/kl_album/uploadpic/'));
			}else{
			...
			{
				$log_photo_cover = substr($log_photo_row['filename'], strpos($log_photo_row['filename'], 'content/uploadpic/'), strlen($log_photo_row['filename'])-strpos($log_photo_row['filename'], 'content/plugins/kl_album/uploadpic/'));
			}else{
				$log_photo_cover = 'images/no_cover_s.jpg';
			}
		}
		...
		<img src="'.BLOG_URL.$log_photo_cover.'" width="60" height="60"></a>
		</li>';
	}
}
//显示单个相册里的照片
	...
	foreach ($log_photo_info as $value)
	{
		...
		if($value['addtime'] == $album)
		{
			if($value['restrict'] == 'private' && ROLE != 'admin')
			{
				...
				$addPhotoFun = Option::EMLOG_VERSION >= '5.1.0' ? 'addPhotoNew' : 'addPhoto';
				while($log_photo = $DB->fetch_array($log_photo_query))
				{
					...
					<br /><a href=\"javascript:{$addPhotoFun}('".str_replace('thum-', '',$log_photo_url)."','{$log_photo_size['w']}','{$log_photo_size['h']}','{$log_photo['description']}');\">嵌入</a>
					</li>";
				}
			}
			break;
